    ...
    NestConfigModule.forRoot({
      ...
      validationSchema: Joi.object({
        ...
        LOG_LEVEL: Joi.string()
          ...
          .default('info'),
        // Max time (ms) a service waits for in-flight work to drain on
        // SIGTERM/SIGINT before force-exiting (graceful-shutdown.service.ts).
        SHUTDOWN_TIMEOUT_MS: Joi.number().default(30000),

        // --- Security ---
        SECRET_KEY: Joi.string().required(),
        // 32-byte hex string (64 chars) used to AES-256-GCM encrypt Access Key secret keys at rest (iam-bc).
        ACCESS_KEY_SECRET_ENCRYPTION_KEY: Joi.string()
          .length(64)
          .hex()
          .required(),
