...
/**
 * Adds the `page:view_print_templates` UI-plane permission gating the new
 * Print Templates admin page (sidebar nav item + `bootAdminPage({
 * pagePermission: ... })` redirect guard). UI-plane permissions are manually
 * curated (never touched by `permissions:sync`, see CLAUDE.md) — mirrors
 * `SeedAccessKeysUiPermission1784526821534`.
 */
const NEW_UI_PERMISSIONS: Array<[string, string, string, string, string]> = [
  [
    ...
    'เข้าหน้าเทมเพลตพิมพ์เอกสาร',
    'View print templates page',
  ],
];
...
export class SeedPrintTemplatesUiPermission1785291692317 implements MigrationInterface {
  name = 'SeedPrintTemplatesUiPermission1785291692317';
